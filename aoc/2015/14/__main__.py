...
def read_input(input_data):
    ...
    stats = np.array([
        tuple(map(np.int32, pattern.findall(line))) for line in input_data.splitlines()
    ])
...
def reindeer_race(reindeer_stats, total_time):
    ...
    total_fly_time = full_cycles * fly_durations + np.minimum(remaining_time, fly_durations)
