...
def expand_universe(universe: np.ndarray):
    ...
    universe = np.insert(universe, np.where(i_non_galaxy_rows)[0], universe[i_non_galaxy_rows], axis=0)
    i_non_galaxy_columns = np.all(universe == space, axis=0)
    universe = np.insert(universe, np.where(i_non_galaxy_columns)[0], universe[:, i_non_galaxy_columns], axis=1)
    return universe
...
def shortest_path_between_galaxies(input_str):
    ...
    return sum([abs(gal_1[0]-gal_2[0])+abs(gal_1[1]-gal_2[1]) for gal_1, gal_2 in galaxies_pairs])
