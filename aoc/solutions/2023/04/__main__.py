...
def process_card(card_str):
    ...
    card_to_list = [
        re.sub(r"\s+", " ", part).strip().split(" ")
        for part in re.split(pattern, card_str_no_multi_spaces)
    ]
    return {
        "_id": card_to_list[0][1],
        "winners": card_to_list[1],
        "card_numbers": card_to_list[2],
        "card_count": 1,
    }
...
def find_card_winners(card_dict):
    return [
        number for number in card_dict["card_numbers"] if number in card_dict["winners"]
    ]
...
def count_all_points(input_str):
    ...
    return sum(
        [evaluate_card_points(processed_card) for processed_card in processed_cards]
    )
