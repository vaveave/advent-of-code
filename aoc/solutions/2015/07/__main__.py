...
def parse_input_wires(str_: str) -> list:
    ...
    if any(check_bitwise_op):
        operation = [
            bitwise_op[c] for c in range(len(bitwise_op)) if check_bitwise_op[c]
        ][0]
...
def part_1(input_data):
    ...
    while len(solutions) < len(circuit):
        for wire, operation in circuit.items():
            if all(input_.isnumeric() for input_ in operation[0]) | all(
                input_ in solutions for input_ in operation[0] if not input_.isnumeric()
            ):
...
def part_2(input_data):
    ...
    while len(solutions) < len(circuit):
        for wire, operation in circuit.items():
            if wire == "b":
                solutions[wire] = 3176
            elif all(input_.isnumeric() for input_ in operation[0]) | all(
                input_ in solutions for input_ in operation[0] if not input_.isnumeric()
            ):
                sol = calc_gate(operation, solutions)
                solutions[wire] = sol
