...
def parse_input(disk_map):
    file_lengths = np.array(
        [int(disk_map[i]) for i in range(len(disk_map)) if i % 2 == 0]
    )
    free_space_lengths = np.array(
        [int(disk_map[i]) for i in range(len(disk_map)) if i % 2 == 1]
    )
...
def calculate_checksum(disk_status):
    return sum(
        position * int(block)
        for position, block in enumerate(disk_status)
        if block != "."
    )
...
def part_2(file_lengths, free_space_lengths):
    ...
    while i >= 0:
        ...
        i_begin_empty_slot = find_consecutive_dots(
            disk_status[: file_id_start + 1], file_id_len
        )
        if i_begin_empty_slot == -1:
            ...
            (
                disk_status[i_begin_empty_slot : i_begin_empty_slot + file_id_len],
                disk_status[file_id_start + 1 : i + 1],
            ) = (
                disk_status[file_id_start + 1 : i + 1],
                disk_status[i_begin_empty_slot : i_begin_empty_slot + file_id_len],
            )
